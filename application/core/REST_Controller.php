...
abstract class REST_Controller extends CI_Controller {
    ...
    // Informational
    const HTTP_CONTINUE = HTTP_CONTINUE;
    const HTTP_SWITCHING_PROTOCOLS = HTTP_SWITCHING_PROTOCOLS;
    const HTTP_PROCESSING = HTTP_PROCESSING;            // RFC2518
    // Success
    /**
     ...
     */
    const HTTP_CREATED = HTTP_CREATED;
    const HTTP_ACCEPTED = HTTP_ACCEPTED;
    const HTTP_NON_AUTHORITATIVE_INFORMATION = HTTP_NON_AUTHORITATIVE_INFORMATION;
    /**
     * The server successfully processed the request, though no content is returned
     */
    const HTTP_NO_CONTENT = HTTP_NO_CONTENT;
    const HTTP_RESET_CONTENT = HTTP_RESET_CONTENT;
    const HTTP_PARTIAL_CONTENT = HTTP_PARTIAL_CONTENT;
    const HTTP_MULTI_STATUS = HTTP_MULTI_STATUS;          // RFC4918
    const HTTP_ALREADY_REPORTED = HTTP_ALREADY_REPORTED;      // RFC5842
    const HTTP_IM_USED = HTTP_IM_USED;               // RFC3229
    // Redirection
    const HTTP_MULTIPLE_CHOICES = HTTP_MULTIPLE_CHOICES;
    const HTTP_MOVED_PERMANENTLY = HTTP_MOVED_PERMANENTLY;
    const HTTP_FOUND = HTTP_FOUND;
    const HTTP_SEE_OTHER = HTTP_SEE_OTHER;
    /**
     * The resource has not been modified since the last request
     */
    const HTTP_NOT_MODIFIED = HTTP_NOT_MODIFIED;
    const HTTP_USE_PROXY = HTTP_USE_PROXY;
    const HTTP_RESERVED = HTTP_RESERVED;
    const HTTP_TEMPORARY_REDIRECT = HTTP_TEMPORARY_REDIRECT;
    const HTTP_PERMANENTLY_REDIRECT = HTTP_PERMANENTLY_REDIRECT;  // RFC7238
    // Client Error
    /**
    ...
    const HTTP_PAYMENT_REQUIRED = HTTP_PAYMENT_REQUIRED;
    /**
     ...
     */
    const HTTP_NOT_ACCEPTABLE = HTTP_NOT_ACCEPTABLE;
    const HTTP_PROXY_AUTHENTICATION_REQUIRED = HTTP_PROXY_AUTHENTICATION_REQUIRED;
    ...
    const HTTP_GONE = HTTP_GONE;
    const HTTP_LENGTH_REQUIRED = HTTP_LENGTH_REQUIRED;
    const HTTP_PRECONDITION_FAILED = HTTP_PRECONDITION_FAILED;
    const HTTP_REQUEST_ENTITY_TOO_LARGE = HTTP_REQUEST_ENTITY_TOO_LARGE;
    const HTTP_REQUEST_URI_TOO_LONG = HTTP_REQUEST_URI_TOO_LONG;
    const HTTP_UNSUPPORTED_MEDIA_TYPE = HTTP_UNSUPPORTED_MEDIA_TYPE;
    const HTTP_REQUESTED_RANGE_NOT_SATISFIABLE = HTTP_REQUESTED_RANGE_NOT_SATISFIABLE;
    const HTTP_EXPECTATION_FAILED = HTTP_EXPECTATION_FAILED;
    const HTTP_I_AM_A_TEAPOT = HTTP_I_AM_A_TEAPOT;                                               // RFC2324
    const HTTP_UNPROCESSABLE_ENTITY = HTTP_UNPROCESSABLE_ENTITY;                                        // RFC4918
    const HTTP_LOCKED = HTTP_LOCKED;                                                      // RFC4918
    const HTTP_FAILED_DEPENDENCY = HTTP_FAILED_DEPENDENCY;                                           // RFC4918
    const HTTP_RESERVED_FOR_WEBDAV_ADVANCED_COLLECTIONS_EXPIRED_PROPOSAL = HTTP_RESERVED_FOR_WEBDAV_ADVANCED_COLLECTIONS_EXPIRED_PROPOSAL;   // RFC2817
    const HTTP_UPGRADE_REQUIRED = HTTP_UPGRADE_REQUIRED;                                            // RFC2817
    const HTTP_PRECONDITION_REQUIRED = HTTP_PRECONDITION_REQUIRED;                                       // RFC6585
    const HTTP_TOO_MANY_REQUESTS = HTTP_TOO_MANY_REQUESTS;                                           // RFC6585
    const HTTP_REQUEST_HEADER_FIELDS_TOO_LARGE = HTTP_REQUEST_HEADER_FIELDS_TOO_LARGE;                             // RFC6585
    // Server Error
    /**
     ...
     */
    const HTTP_NOT_IMPLEMENTED = HTTP_NOT_IMPLEMENTED;
    const HTTP_BAD_GATEWAY = HTTP_BAD_GATEWAY;
    const HTTP_SERVICE_UNAVAILABLE = HTTP_SERVICE_UNAVAILABLE;
    const HTTP_GATEWAY_TIMEOUT = HTTP_GATEWAY_TIMEOUT;
    const HTTP_VERSION_NOT_SUPPORTED = HTTP_VERSION_NOT_SUPPORTED;
    const HTTP_VARIANT_ALSO_NEGOTIATES_EXPERIMENTAL = HTTP_VARIANT_ALSO_NEGOTIATES_EXPERIMENTAL;                        // RFC2295
    const HTTP_INSUFFICIENT_STORAGE = HTTP_INSUFFICIENT_STORAGE;                                        // RFC4918
    const HTTP_LOOP_DETECTED = HTTP_LOOP_DETECTED;                                               // RFC5842
    const HTTP_NOT_EXTENDED = HTTP_NOT_EXTENDED;                                                // RFC2774
    const HTTP_NETWORK_AUTHENTICATION_REQUIRED = HTTP_NETWORK_AUTHENTICATION_REQUIRED;
    /**
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        log_data($controller_method);
        if (!method_exists($this, $controller_method)) {
            // $controller_method = "index_" . $this->request->method;
            // array_unshift($arguments, $object_called);
            log_message("error", "NOT FOUND API:{$this->router->class}/index_" . $this->request->method);
            ##这个地方只能是404
            show_404();
            exit;
        }
    ...
    protected function _check_cors()
    {
        ...
        // log_message("test", $allowed_headers);
